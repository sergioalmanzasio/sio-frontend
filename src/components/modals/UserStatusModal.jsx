...
const UserStatusModal = ({ isOpen, onClose, onSubmit, isSubmitting = false, userToEdit = null }) => {
  ...
  useEffect(() => {
    if (isOpen && userToEdit) {
      ...
      const isActive = userToEdit.status && userToEdit.status.includes("Activo");
      setFormData({
        name: userToEdit.name || "",
        username: userToEdit.username || "",
        role_assigned: userToEdit.role_assigned || "",
  ...
  const isClient = (formData.role_assigned || "").toLowerCase().includes("cliente");
                  ...
                  {!isClient && (
                    <div className="pt-2">
                      <div className="flex items-center justify-between p-4 bg-gray-50 border border-gray-100 rounded-lg">
                        <div>
                          <h3 className="text-sm font-medium text-gray-800">Estado de usuario</h3>
