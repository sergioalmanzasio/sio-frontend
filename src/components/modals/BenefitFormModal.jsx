...
const BenefitFormModal = ({ isOpen, onClose, onSubmit, isSubmitting = false, benefitToEdit = null }) => {
    ...
    if (isOpen) {
      ...
      if (benefitToEdit) {
        setFormData({
          ...
          is_active: benefitToEdit.is_active !== undefined ? benefitToEdit.is_active : true
        });
      } else {
        setFormData({
          description: "",
        ...
        <div className="relative inline-block align-bottom bg-white rounded-lg text-left shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-xl w-full z-50">
          <form onSubmit={handleSubmit} className="flex flex-col max-h-[90vh]">
            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 flex-shrink-0">
              <div className="flex justify-between items-center mb-5 pb-4 border-b border-gray-200">
                <h3 className="text-xl leading-6 font-bold text-gray-900" id="modal-title">
                  {benefitToEdit ? "Actualizar beneficio" : "Nuevo beneficio"}
                </h3>
                <button
                  type="button"
            ...
            <div className="mt-8 flex justify-center gap-4 bg-gray-50 px-4 py-3 sm:px-6 rounded-b-lg flex-shrink-0 border-t border-gray-200">
              ...
              >
                {isSubmitting ? "Guardando..." : benefitToEdit ? "Actualizar beneficio" : "Guardar beneficio"}
              </button>
            </div>
          </form>
        </div>
