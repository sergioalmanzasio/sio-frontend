...
const AdminCategoriesTable = () => {
  const { getAllCategories, createCategory, updateCategory, loading } =
    useCategoryAdmin();
  ...
  const SortIcon = ({ column }) => {
    if (sortColumn !== column)
      return <ArrowUpDown className="inline ml-1 h-3.5 w-3.5 opacity-50" />;
    ...
    if (categoryToEdit && categoryToEdit.is_active !== formData.is_active) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          resultProcess = await updateCategory(categoryToEdit.id, formData);
          if (resultProcess && resultProcess.process === "success") {
            ToastAlert({
              position: "top",
              timer: 1800,
              icon: "success",
      ...
      resultProcess = await createCategory(formData);
    }
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {currentCategories.length > 0 ? (
                    ...
                    <tr>
                      ...
                      >
                        No hay categorías registradas o que coincidan con la
                        búsqueda.
                      </td>
                    </tr>
                  )}
